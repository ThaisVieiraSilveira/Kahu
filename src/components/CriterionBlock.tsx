    ...
    >
      ...
      <div className="relative group">
          ...
          <table className="w-full text-left border-collapse min-w-[700px] md:min-w-0">
            <thead>
              <tr className="bg-white">
                <th className="pl-8 pr-4 py-5 text-[11px] font-bold text-neutral-400 uppercase tracking-[0.15em] w-1/4">
                  Colaborador
                </th>
                {SCORES.map((score) => (
                  <th
                    key={score}
                    className="px-1 md:px-2 py-5 text-center text-[11px] font-bold text-neutral-400 uppercase tracking-[0.15em]"
                  >
                    {score}
                  </th>
                ))}
              ...
              {employees
                ...
                <tr key={employee} className="group hover:bg-neutral-50/30 transition-all duration-300">
                  ...
                  {SCORES.map((score) => {
                    ...
                    return (
                      <td key={score} className="px-1 md:px-2 py-5 text-center">
                        <button
                          onClick={() => onChange(employee, score)}
                          className={cn(
                            "w-9 h-9 md:w-11 md:h-11 rounded-lg md:rounded-xl transition-all duration-300 flex items-center justify-center text-xs md:text-sm font-bold border-2 active:scale-90",
                            isSelected
                              ? "bg-brand-600 border-brand-600 text-white shadow-[0_4px_12px_rgba(101,163,13,0.3)] scale-110 z-10"
        ...
        <div className="md:hidden absolute right-0 top-1/2 -translate-y-1/2 pointer-events-none">
          <div className="w-8 h-32 bg-gradient-to-l from-white/80 to-transparent flex items-center justify-end pr-1">
